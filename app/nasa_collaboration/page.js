...
const API_KEY = 'piB8o8lBYGycnf2Fdy2oJ3ZTgyK5cR2r6RtyolP1';
...
const RoverPhoto = ({src, date, roverName}) => {
  return(
    <div className={styles.roverPhotoCard}>
      <h3>{roverName}</h3>
      <p>Date: {date}</p>
      <img src={src} alt={`Photo taken by ${roverName}`} className={styles.image} />
    </div>
  );
};
    ...
    <div className="fullBGpicture">
      <main className="mainContent">
        ...
        <section className="card">
          ...
          {dailyImg.url && <img src={dailyImg.url} alt={dailyImg.title} className={styles.image} /> }
